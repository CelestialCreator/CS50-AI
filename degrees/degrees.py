...
def shortest_path(source, target):
    ...
    # Keep searching until we find target or exhaust all possibilities
    while True:
        ...
        # Check all neighbors (people who starred in movies with current person)
        for movie_id, person_id in neighbors_for_person(node.state):
            # If we haven't seen this person before
            if not frontier.contains_state(person_id) and person_id not in explored:
                # Check if this is our target
                if person_id == target:
                    # We found the target! Construct the path
                    path = []
                    # Create a node for our final state
                    current = Node(
                        state=person_id,
                        parent=node,
                ...
                # If not target, add to frontier to explore later
                child = Node(
                    state=person_id,
                    parent=node,
                    action=(movie_id, person_id)
